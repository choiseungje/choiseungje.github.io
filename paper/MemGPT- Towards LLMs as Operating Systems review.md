...
2. 메모리 효율적인 그래디언트 체크포인팅 (Gradient Checkpointing)
	- Transformer 모델의 학습 과정에서, **중간 연산 결과를 저장하지 않고 필요할 때 다시 계산하는 방식**으로 메모리 사용량을 최적화
	-  특히 `Recall Storage` 및 `Archival Storage`에서 필요한 데이터만 불러오도록 구성하여, GPU 메모리 사용량을 줄이고 모델의 확장성을 극대화.
